...
?>
	<div id="social-tabs-comments">
		...
		<ul class="social-nav social-clearfix">
			<li class="social-all social-tab-main<?php echo (!isset($_GET['social_tab']) ? ' social-current-tab' : ''); ?>"><a href="#" rel="social-all"><span><?php comments_number(__('0 ответов', 'social'), __('1 ответ', 'social'), __('% ответов', 'social')); ?></span></a></li>
			<li class="social-wordpress<?php echo ((isset($_GET['social_tab']) and $_GET['social_tab'] == 'wordpress') ? ' social-current-tab' : ''); ?>"><a href="#" rel="wordpress"><span><?php printf(_n('1 комментарий', '%1$s комментариев', (isset($groups['wordpress']) ? $groups['wordpress'] : 0), 'social'), (isset($groups['wordpress']) ? $groups['wordpress'] : 0)); ?></span></a></li>
			<li class="social-twitter<?php echo ((isset($_GET['social_tab']) and $_GET['social_tab'] == 'social-twitter') ? ' social-current-tab' : ''); ?>"><a href="#" rel="social-twitter"><span><?php printf(_n('1 твит', '%1$s твитов', (isset($groups['social-twitter']) ? $groups['social-twitter'] : 0), 'social'), (isset($groups['social-twitter']) ? $groups['social-twitter'] : 0)); ?></span></a></li>
		...
		<div id="social-comments-tab-all" class="social-tabs-panel social-tabs-first-panel">
			<div id="comments" class="social-comments">
				<?php
				if ($last_reply_time) {
					echo '<div class="social-last-reply-when">'.sprintf(__('Последний ответ был %s', 'social'), Social_Date::span_comment($last_reply_time)).'</div>';
				}
				if (count($social_items)) {
				...
				<nav id="comment-nav-below">
					<h1 class="assistive-text"><?php _e('Комментарии', 'social'); ?></h1>
					<div class="nav-previous"><?php previous_comments_link(__('&larr; Старые комментарии', 'social')); ?></div>
					<div class="nav-next"><?php next_comments_link(__('Новые комментарии &rarr;', 'social')); ?></div>
				</nav>
				<?php endif; ?>
			</div>
		</div>
