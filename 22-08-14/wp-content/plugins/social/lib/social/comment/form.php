...
final class Social_Comment_Form {
	...
	public function configure_args(array $default_args) {
		...
		$fields = array(
			'author' => $this->to_input_group(__('Имя', 'social'), 'author', $commenter['comment_author'], $req),
			'email' => $this->to_input_group(__('Email', 'social'), 'email', $commenter['comment_author_email'], $req, __('Не опубликовано', 'social')),
			'url' => $this->to_input_group(__('Сайт', 'social'), 'url', $commenter['comment_author_url'])
		);

		$args = array(
			'label_submit' => __('Запостить', 'social'),
			'title_reply' => __('Профиль', 'social'),
			'title_reply_to' => __('Запостить ответ %s', 'social'),
			'cancel_reply_link' => __('отмена', 'social'),
			'comment_notes_after' => '',
			'comment_notes_before' => '',
			'fields' => $fields,
			'comment_field' => $this->to_textarea_group(__('', 'social'), '', '', true, 'textarea')
		);

		if ($this->is_logged_in) {
			$override = array(
				'title_reply' => __('Оставить комментарий', 'social')
			);
