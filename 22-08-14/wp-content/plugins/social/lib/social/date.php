...
class Social_Date {
	...
	public static function span_comment($remote, $local = NULL) {
		...
		elseif (!empty($span['months'])) {
			if ($span['months'] == '1') {
				$output = __('1 месяц назад', 'social');
			}
			else {
				$output = sprintf(__('%s месяцев назад', 'social'), $span['months']);
			}
		}
		...
		elseif (!empty($span['weeks'])) {
			if ($span['weeks'] == '1') {
				$output = __('1 неделю назад', 'social');
			}
			else {
				$output = sprintf(__('%s недель назад', 'social'), $span['weeks']);
			}
		}
		...
		elseif (!empty($span['days'])) {
			if ($span['days'] == '1') {
				$output = __('1 день назад', 'social');
			}
			else {
				$output = sprintf(__('%s дней назад', 'social'), $span['days']);
			}
		}
		...
		elseif (!empty($span['hours'])) {
			if ($span['hours'] == '1') {
				$output = __('1 час назад', 'social');
			}
			else {
				$output = sprintf(__('%s часов', 'social'), $span['hours']);
			}
		}
		...
		elseif (!empty($span['minutes'])) {
			if ($span['minutes'] == '1') {
				$output = __('1 минуту назад', 'social');
			}
			else {
				$output = sprintf(__('%s минут назад', 'social'), $span['minutes']);
			}
		}

		// Seconds
		else {
			$output =  __('прямо сейчас', 'social');
		}
